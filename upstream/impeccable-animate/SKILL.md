...
description: "Add purposeful motion to a user interface. Use when a design needs animation that explains state, relationship, and hierarchy — not decoration, but motion that carries meaning. Covers motion thesis, material-by-meaning, timing and easing, implementation strategy, and accessibility."
license: Apache-2.0
---
...
Respect autoplay and sound preferences. Any nonessential loop must stop when offscreen or hidden. Respect the user's `prefers-reduced-motion` setting — provide a static fallback for all animated content.
