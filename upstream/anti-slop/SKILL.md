...
name: anti-slop
description: >-
  Anti-AI-slop design and copy rules for any UI work (website, web app, or
  interface). Use whenever building, refining, or auditing user-facing UI.
  Holds every visual decision to a purpose test and the result to a liveliness
  bar so the output feels crafted by a designer, not generated by AI. Two
  modes: DURING (apply rules while building) or AFTER (audit finished work
  with a numbered findings list). Pairs with a DESIGN.md / brand direction for
  identity. Complements hallmark, taste-skill, baseline-ui, and improve-ui.
license: MIT
metadata:
  version: "1.0"
  source: miqdadbadjuber/anti-slop (MIT)
  port: clean methodology-only port of upstream antislop.md (no scripts, no deps)
  relates: hallmark, taste-skill, baseline-ui, improve-ui, huashu-design
---
...
This is a clean methodology port of `antislop.md` from [miqdadbadjuber/anti-slop](https://github.com/miqdadbadjuber/anti-slop) (MIT). No scripts, assets, or dependencies are bundled; only the prose methodology is retained. Complements the bundle's `hallmark`, `taste-skill`, `baseline-ui`, `improve-ui`, and `huashu-design` skills.
